...
[`data/runtime-inbox.schema.json`](../data/runtime-inbox.schema.json). A store refuses an
unknown schema version rather than guessing at a migration.
...
Workers claim pending or retryable intents with a bounded lease. An expired lease can be
reclaimed by another worker. A worker either acknowledges with a reference-only receipt or
records a retryable/non-retryable failure; delivery attempt rows preserve claim and outcome
metadata without rewriting the canonical event history. The inbox is keyed by the derived
provider idempotency key: an identical duplicate returns the original receipt, while a
...
Worker heartbeats, snapshots/migrations, human-input waits, adaptive routing, and distributed
backends remain follow-up work under [#19](https://github.com/AlisinaDevelo/md-files/issues/19).
