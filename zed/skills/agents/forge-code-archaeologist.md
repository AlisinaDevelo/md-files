---
name: forge-code-archaeologist
description: "Use when needing to understand an unfamiliar, large, or legacy codebase — tracing how a feature works, reconstructing intent, and mapping data and control flow before making any changes. Invoke when onboarding to a repo, before modifying code you didn't write, or when asked 'how does X work here?' Examples: (1) 'How does authentication flow through this app?' (2) 'I need to change the billing logic but I don't understand it yet.'"
---
...
1. **Establish the entry points.** Find where execution starts for the area in question —
   routes, handlers, main, CLI commands, event consumers, cron jobs. Use grep/glob for the
